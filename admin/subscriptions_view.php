...
<html>
    <head>
        ...
        <title>Delites | Subscriptions</title>
...
<body style="margin-left:25%; width:75%">          
    ...
    <table>
    	...
    	<tbody>
    		<?php
    			$query=mysqli_query($mysqli,"select * from subscribed")or die(mysqli_error($mysqli));
    			while ($row=mysqli_fetch_array($query)){
    			$email=$row['email'];
    			
    			$queryc=mysqli_query($mysqli,"select * from customer where email='".$email."'")or die(mysqli_error($mysqli));
    			if($rowc=mysqli_fetch_array($queryc)){
    				$custname=$rowc['name'];
    			}
    			else{ $custname="Not Registered"; }
    		?>
    		<tr>
